...
import { LEGACY_BASELINE_MACHINE_DATA_MARKER, ROOT, parseMachineData, readArtifact } from './lib/validate-capability-audit-core.mjs';
...
try {
  ...
  assert.ok(markdown.startsWith(`<!-- ${LEGACY_BASELINE_MACHINE_DATA_MARKER}:capability-audit-input:v1 -->\n\`\`\`json\n`));
  ...
  assert.match(bareMarkdown.stderr, new RegExp(`bare JSON Markdown artifact requires ${LEGACY_BASELINE_MACHINE_DATA_MARKER} envelope`));
