...
const GRAPH_PROJECT = 'ontopolis-baseline-26026fb';
...
function discoverRustAnalyzer() {
  const main = execFileSync('git', ['worktree', 'list', '--porcelain'], { cwd: ROOT, encoding: 'utf8' }).split('\n\n').map((block) => Object.fromEntries(block.split('\n').map((line) => { const [key, ...rest] = line.split(' '); return [key, rest.join(' ')]; }))).find((entry) => entry.branch === 'refs/heads/main')?.worktree;
  if (!main) fail('main worktree is required to resolve rust-analyzer');
  const executable = execFileSync('rustup', ['which', 'rust-analyzer'], { cwd: main, encoding: 'utf8' }).trim();
  const probe = spawnSync(executable, ['--version'], { cwd: main, encoding: 'utf8' });
...
function startLspRuntime() {
  ...
  lspRuntimeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ontopolis-audit-lsp-'));
...
async function warmLspRuntime(worktree) {
  ...
  for (; attempts < 2; attempts++) {
    response = await lspDocumentSymbols(path.join(worktree, 'crates/ontopolis-types/src/coords.rs'), worktree, { timeoutMs: 60000 });
...
async function main() {
  ...
  if (process.argv.includes('--lsp-lifecycle-smoke')) {
    ...
    let lifecycle;
    try {
      const response = await directRustDocumentSymbols(path.join(original.worktree, 'crates/ontopolis-types/src/coords.rs'), original.worktree);
  ...
  const sourceDefinition = runCapture({ runId, baseline, bootstrap, output, receiptId: 'task-04-source-definition', adapter: 'source_definition', argv: ['git', 'show', `${baseline}:crates/ontopolis-types/src/coords.rs`] });
  const testList = runCapture({ runId, baseline, bootstrap, output, receiptId: 'task-04-rust-test-list', adapter: 'cargo_test_list', argv: ['cargo', 'test', '-p', 'ontopolis-types', '--test', 'coords', '--', '--list'] });
  const testResults = runCapture({ runId, baseline, bootstrap, output, receiptId: 'task-04-rust-test-results', adapter: 'exact_test', argv: ['cargo', 'test', '-p', 'ontopolis-types', '--test', 'coords', 'world_to_chunk_local_roundtrip', '--', '--exact'] });
  ...
  const testPath = 'crates/ontopolis-types/tests/coords.rs', testBlob = files.find((file) => file.path === testPath)?.blob_oid; if (!testBlob) fail('baseline coordinate test not selected');
  ...
  const tests = testRows.map((test) => ({ test_id: `task-04-${test.test_name.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase()}`, package: 'ontopolis-types', target_kind: 'integration-test', target_name: 'coords', test_name: test.test_name, path: testPath, blob_oid: testBlob, line_start: test.line_start, line_end: test.line_end, definition_character: test.definition_character, test_root_qn: testRootQn(test), eligibility: test.eligibility, discovered_receipt_id: testList.receipt.receipt_id }));
  ...
  const reconciliation = seal({ schema_version: 1, run_id: runId, source_baseline_sha: baseline, targets: [{ package: 'ontopolis-types', target_kind: 'integration-test', target_name: 'coords', path: testPath, blob_oid: testBlob, eligibility: 'exact_test', rationale: 'the declared Cargo integration target is the immutable baseline coords test source' }], tests, mappings, completeness: { status: 'complete', evidence_ids: ['e-source-definition'], rationale: 'all cargo-discovered tests are recorded; every LSP-linked baseline test is mapped and remaining tests are explicitly non_testable_target' } });
  ...
  const independentReplayProjection = { baseline_tree_oid: git(['rev-parse', `${baseline}^{tree}`]).trim(), source_listing_sha256: bytesDigest(git(['ls-tree', '-r', '-z', baseline], null)), graph_schema_sha256: digest(replaySchema), graph_queries: replayQueries, cargo_test_list_stdout_sha256: replayCargo(['test', '-p', 'ontopolis-types', '--test', 'coords', '--', '--list']), cargo_test_results_stdout_sha256: replayCargo(['test', '-p', 'ontopolis-types', '--test', 'coords', 'world_to_chunk_local_roundtrip', '--', '--exact']), lsp: lspProjection([...replayRust, ...replayNonRust]) };
