...
pub fn atomic_write(path: &Path, bytes: &[u8]) -> Result<(), PersistenceError> {
    let parent = path.parent().ok_or_else(|| {
        PersistenceError::codec("path has no parent directory")
    })?;
    let file_name = path.file_name().ok_or_else(|| {
        PersistenceError::codec("path has no file name")
    })?;
    ...
    let mut file = fs::File::create(&temp_name).map_err(|e| {
        PersistenceError::codec(format!("create temp file: {e}"))
    })?;
    file.write_all(bytes).map_err(|e| {
        PersistenceError::codec(format!("write temp file: {e}"))
    })?;
    file.flush().map_err(|e| {
        PersistenceError::codec(format!("flush temp file: {e}"))
    })?;
    drop(file);

    fs::rename(&temp_name, path).map_err(|e| {
        PersistenceError::codec(format!("atomic rename: {e}"))
    })?;
...
pub fn read_snapshot_file(path: &Path) -> Result<SnapshotEnvelope, PersistenceError> {
    let bytes = fs::read(path).map_err(|e| {
        PersistenceError::codec(format!("read file: {e}"))
    })?;
