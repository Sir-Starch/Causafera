...
- Closed `TODO-THERMAL-006` (`plans/thermal-conservation-aggregate-validation.md`): `RuntimeState::import_snapshot` now cross-validates every `ThermalConservationReceipt`'s aggregate literals (`total_cell_energy_before/after`, `total_reservoir_budget_before/after`, `total_material_retained_before/after`) against checked `i128` sums of the actual imported field energies, reservoir budgets, and material surface retained energies. The validation enforces identities I1–I6 (including the new I2 cell-delta-with-injection, I3 material flux, I5 batch chain, and I6 terminal anchor against materialized state) and rejects untouched-cell tampering that previously went undetected. No encoded byte changed, no section-major bump, and no digest-schema bump: `THERMAL_SECTION_MAJOR` stays 2, `MATERIAL_SURFACE_SECTION_MAJOR` stays 3, and `CURRENT_DIGEST_SCHEMA_VERSION` stays 6. The same statistical harness on an AMD Ryzen 9 7950X3D measured the `production_loop_config(2026)` snapshot (36 batches, 27 cells, 668 transfer receipts) before and after the validator: mean import time `1052697.1 ns` → `955733.4 ns` (`-9.21%`), median `942363 ns` → `949907 ns` (`+0.80%`), with sample standard deviations `177317.4 ns` and `24939.0 ns`; no performance pass/fail claim is made because no regression threshold was defined. A uniform aggregate offset alone is rejected by I6. The remaining limitation requires a coordinated `+Δ` shift to the materialized final field and every batch's cell totals; the bootstrap total `C_1^-` has no independent persisted anchor, so that coordinated forgery remains inside the `SECURITY.md` pre-alpha untrusted-snapshot carve-out.
...
- Made agent guidance and planning documentation tool-neutral and canonical across `AGENTS.md`, `CLAUDE.md`, and `PLANS.md`: established an explicit source-authority order, replaced tool-specific orchestration and code-discovery requirements with neutral practices, structured `PLANS.md` into a clear index separating active work from completed implementation records, and kept Foundation Era phases as historical provenance rather than current guidance;
