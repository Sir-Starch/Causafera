...
- Bounded seam-delivered mana at the field's own saturation ceiling (`TODO-MANA-006`, `plans/mana-seam-saturation-ceiling.md`). `diffuse_cell` clamped a cell fed from inside its own chunk to `0..=maximum_intensity` at the end of its own computation, but `apply_exchange_delta` added a cross-seam share straight into the receiving proposal with no ceiling of its own; measured on two adjacent extent-3 chunks with every cell seeded at a `maximum_intensity` of 1 000, seam-fed cells finished above it. A refund-to-giver or persisted-ledger design was considered, per the TODO's own framing, and rejected: the interior path's clamp already discards excess without refunding a contributing neighbour when it engages, and the seam conservation test (`diffusion_alone_conserves_mana_across_a_seam`) sets `maximum_intensity = i64::MAX / 4` and never exercises the clamp at all, so a plain clamp does not in fact reopen `TODO-MANA-002`'s conservation defect (a different mechanism — truncation loss, not ceiling loss). `apply_exchange_delta` now clamps its running total to `0..=maximum_intensity` after every delivery, the same bound the interior path already applies once; because addition is commutative, this is order-independent even where several distinct neighbouring chunks feed one chunk-corner or -edge cell at once, dissolving the "which giver is refused" problem without a new rule. Verified inert at production scale: physical digest, history digest, total mana and every behaviour metric are byte-identical before and after the fix across all six standard seeds and both gate configurations at the production `chunk_extent` of 3, compared via a throwaway `git worktree` checkout of the pre-fix commit.
...
- Replaced the blanket prohibition on external code and documentation contributions with a CLA-gated policy: external contributions may be merged after CLA acceptance, the required checks, and maintainer approval. `CONTRIBUTING.md` documents the eight-step flow from open TODO through CLA acceptance to merge;
...
- Brought the CLA acceptance workflow into service and closed `TODO-LEGAL-001`. CLA 1.1 is published as public Gist `eb32d78ea648f989831f7aa0a3bac81c` at revision `7c6daa72020318c47d14bca27655097cce236d6b`, byte-identical to `CLA.md` (15587 bytes, SHA-256 `d32a10df…`, blob `3c896929…`); CLA Assistant is linked to `Sir-Starch/Causafera` against that Gist's `CLA.md`, with Shared Gist disabled and no minimum file-count or line-count exemption. Acceptance was verified end to end: the `license/cla` check reports pending for an unsigned contributor and moves to passing on acceptance without a further push. `license/cla` is now a **required status check on `main`** in the `Protect main` ruleset, alongside `rust` and `ui`, with strict up-to-date branches, branch-deletion and non-fast-forward protection preserved and no bypass actors — so an unsigned contribution cannot be merged. The maintainer and non-signing automation accounts are exempt through CLA Assistant's own allowlist, the narrowest available mechanism: it suppresses the signature request for named accounts and changes nothing else, leaving `rust`, `ui`, and CLA enforcement fully intact for every other contributor. A repository-admin ruleset bypass was deliberately not used, since it would have bypassed the build and test gates as well. External contributions may now be merged after CLA acceptance, required checks, and maintainer approval; opening a pull request still does not by itself accept the CLA;
- Closed the "is a chunk a material?" ambiguity in the local-detail contract. `spatial-hierarchy.md`'s `Spatial Chunk` definition guessed a physical size ("candidate: 1 km²") and listed hydrology and ecology as chunk-resident state; neither claim has code backing — `CHUNK_SIZE` (`crates/causafera-types/src/coords.rs`) is a dimensionless lattice-cell count with no metre-per-cell constant anywhere in the workspace, and hydrology/ecology remain M0 documentation-only per the coverage matrix. Rewrote the section to state the chunk as an addressing/storage/computation/causal-resolution unit and cite the independent resolutions that already coexist inside one chunk today: a `CHUNK_SIZE × CHUNK_SIZE` terrain height field alongside a separately configured mana `chunk_extent` volume. Extended INV-043 with a "not a material or physically indivisible cell, no privileged metric scale" clause and INV-037 with an "independent per-aspect resolution inside one chunk" clause, rather than adding a new invariant number. Tightened `coordinates.md`'s local-frame section to state that a `LocalMetricFrame` (parcel/structure/interior/body/object) is bounded only by its own `maximum_offset_mm`, independent of `CHUNK_SIZE` or the mana lattice, and added the same cross-reference to `buildings.md`'s interior-spaces section and `parcels.md`'s own record description. Added a one-line disclaimer to `terrain.md`'s Level of Detail ladder that it describes terrain's own resolution and does not bound other domain aspects. Extended RFC-RES-001's no-synthesis rule from individuals to objects and structures, matching INV-039. Recorded `TODO-WORLD-003`, a bounded future validation slice (one chunk, one parcel, one structure, several interiors, real local coordinates, thermal/mana interaction, persistence, provenance, one additive observer projection) that explicitly excludes city generation, indoor physics, and a general promotion/demotion system. Nothing here changes `CHUNK_SIZE`, production code, persistence, the observer protocol, or current Detailed Development priority — `docs/roadmap/roadmap.md` records the slice as scoped but not scheduled;
- Closed `TODO-THERMAL-002`, giving a material surface's retained thermal energy a physically meaningful response to thermal exposure: a bounded, conserved exchange with its co-located `ThermalField` cell, a third conserved bucket alongside cells and reservoirs. The exchange runs inside the same atomic Physics-phase batch that already diffuses cell-to-cell energy, using the identical signed-flux formula as face diffusion; the coefficient bound widened from `6 * transfer_fraction <= scale` to `6 * transfer_fraction + material_exchange_fraction <= scale` so a cell cannot go negative even with all seven simultaneous outflows realized in one tick, and headroom capping applies only to the heating direction. No new scheduler `System` was registered and no RNG stream was added — confirmed via `causafera-core/src/scheduler.rs` that RNG stream keys derive from registration order, a namespace distinct from the `*_SYSTEM_ID` constants used only for `EventProposalKey` ordering, which is what makes the additive integration correct rather than merely convenient. **Two consumer-visible breaks**: `CURRENT_DIGEST_SCHEMA_VERSION` moved 5→6 (a material surface's retained energy and its trace anchor are now digest inputs), and the observer world snapshot's `material_surface_delta_schema_version` moved 3→4 (a new `MaterialSurfaceThermalDelta` list shares that version field with the existing condition/gate deltas, since all three address the same `MaterialSurfaceId` family). Persistence extends the existing signed-flux receipt equation to include the material term, so a coordinated forgery is rejected in any batch, not only the latest. Added Explanation claim schema 17 (`MATERIAL_SURFACE_THERMAL_EXCHANGE_SCHEMA`), answered by a standalone query rather than folded into the existing condition/mana loop explanation, because thermal exchange runs independent of contact and folding in would have made a thermally-active-but-never-touched surface unexplainable; an unknown surface ID is rejected as invalid, while a real surface with no recorded exchange yet answers `Unknown` rather than erroring. Production defaults (`transfer_fraction = 128`, `material_exchange_fraction = 64`, `material_thermal_capacity = 1024`) are a foundation-phase choice for observability of the new coupling, not a tuned physical constant. Per-tick performance cost was not benchmarked in this tranche; the material record adds a small, bounded per-cell increment to `TODO-PERF-002`/`TODO-PERF-003`'s already-named unbounded thermal-receipt growth, which this closure does not resolve. Opened `TODO-THERMAL-007` (expansion/damage/phase-change response to retained heat) and `TODO-THERMAL-008` (heterogeneous per-material thermal properties from `causafera_types::Material`) as explicit follow-ups this tranche deliberately left out of scope. See `plans/thermal-material-surface-coupling.md` for the full design, Decision log, and verification mapping;
- Rewrote agent guidance to be concise, current, and tool-neutral. `AGENTS.md` is now the canonical repository-wide guide for any coding agent, structured as project purpose, scope and authority, current development model, task-scoped reading, planning, non-negotiable architecture, implementation practice, code discovery, Git safety, validation, documentation, and completion reporting. It states an explicit authority order — invariants/ADRs/RFCs/canonical ExecPlans define intended contracts, current code and tests define implemented behaviour, roadmap/maturity matrix/backlog/changelog describe status, and generated summaries, agent memory, tool indexes, chat history and completed plans are not authoritative — and requires conflicts between sources to be resolved explicitly rather than silently. It replaces the fixed "read these eight documents before any change" list with task-scoped reading through `docs/index.md`, and replaces the embedded copies of the cognition and spatial subsystem specifications with a summary of the broadly applicable invariants linking to `docs/architecture/invariants.md` for the full INV-001–INV-043 contract. Foundation Era phases 0–26 are stated once as completed historical provenance that does not carry present implementation priority; current priorities come from active plans, the roadmap, the backlog, and maturity evidence. Determinism, provenance and conservation, the Ground Truth/cognition boundary, the no-semantic-labels rule, mana-reads-structure, chunks-are-addressing, non-authoritative Explanation/observer/UI/LLM layers, digests-are-identities, no-fixtures-in-production, modular architecture, and deliberate schema versioning are all preserved, as is the full Git-safety policy including the accepted-plan checkpoint procedure — with the delegation rule phrased generically rather than around named orchestration agents;
- Reduced `CLAUDE.md` to a thin Claude Code adapter that keeps the `@AGENTS.md` import and covers only harness specifics: no external orchestration framework is required, framework-specific agents and commands are not invoked unless the current session asks, Claude memory and compacted context are navigation hints rather than authoritative sources, `codebase-memory-mcp` is optional rather than mandatory-first, and after a resume or compaction the active plan's Progress and Decision Log plus `git status` are re-read;
- Removed tool-specific planning requirements from canonical documentation. `PLANS.md`'s plan-locations section no longer names `.omo/plans/`, Momus review paths, or `start-work` ledger references; it now states neutrally that `plans/` is the sole source of truth and that any tool-specific execution artifact outside it is a non-authoritative adapter whose findings become project decisions only once incorporated into the canonical plan. Historical records under `plans/` that mention the tooling used at the time were deliberately left intact as provenance;
- Made code discovery tool-neutral across the documentation. `codebase-memory-mcp` is no longer a repository requirement in `AGENTS.md`, `CLAUDE.md`, `docs/development/codebase-memory.md`, or `docs/development/contributing.md`; exact text search, LSP, call hierarchy, or any available semantic graph are all acceptable, queries should be narrow, the same question should not be re-asked through two mechanisms without a verification reason, and current source must be read before relying on a generated index. `docs/index.md` also drops its hardcoded list of currently active plans in favour of linking `AGENTS.md` and `PLANS.md`.
