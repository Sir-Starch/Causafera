...
- Added `GOVERNANCE.md` as the authoritative governance document: Causafera is an independent, author-led, maintainer-governed FOSS project whose canonical repository is directed by the `Sir-Starch` GitHub identity, not by community consensus, contributor vote, or influence proportional to contribution size. Contributing creates no governance rights, technically valid work may be declined for vision fit, and design disagreements are legitimately explored through forks. The document also records why the engine must stay FOSS — independent inspection, reproducibility, verification of causal traces and assumptions, adaptation, and the avoidance of unverifiable "trust the author" claims — while separating source availability from validation, which comes from deterministic replay, tests, provenance, documented assumptions, and reproducible experiments. Six concepts are held apart deliberately: public FOSS licensing, governance authority, copyright ownership, CLA-granted rights, commercial licensing, and scientific or technical validation;
- Released CLA version 1.1. Identified the Project Maintainer through the `Sir-Starch` GitHub identity while stating plainly that this is a documentation convention rather than a legal identification and that pseudonymity is not claimed to provide anonymity; aligned the Public Project License definition with the repository's functional/non-functional split; made the five required acceptance-record elements explicit (authenticated contributor identity, exact CLA version, timestamp, associated pull request or commit, accepting party); stated that commercial or proprietary outbound licensing is additional to, and cannot withdraw, the continuing public availability of accepted contributions; added section 13 confirming that the CLA grants no governance rights and transfers no copyright; and added a version history so acceptance always binds to a specific text. Recorded eight questions reserved for Netherlands-qualified legal review rather than answering them with improvised legal language, and stated explicitly that the CLA has **not** been legally reviewed or approved;
...
- Added `docs/legal/cla-service-setup.md`, a maintainer checklist for the manual work required outside the repository: legal review first, publishing an immutable versioned CLA source, connecting `Sir-Starch/Causafera` to the hosted CLA Assistant service, testing with a real pull request from a second account, verifying the status check, requiring it on `main`, handling automation accounts such as Dependabot, exporting and privately retaining acceptance records, and updating repository status text only after the service is confirmed operational. Nothing in it has been performed; no external GitHub or CLA Assistant setting was configured by this change;
...
- Updated `TODO-LEGAL-001` to target CLA 1.1, the reserved legal questions, and the setup checklist.
