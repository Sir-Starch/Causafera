...
Four items this list previously deferred are now implemented and are no longer open questions: same-chart cross-chunk exchange, threshold/hysteresis field-to-matter effects, and two concrete carrier adapters — the change-driven material surface and the standing terrain structure. What replaced the question is a narrower one, and every part of that narrower question is now settled too. Provenance is settled: a seam cell could commit a change with no causal ancestry, and `TODO-MANA-005` closed that, so no mana cell change is proposed without a cause. Saturation is settled: seam delivery did not apply the field's own saturation ceiling, so a cell fed across a seam in a saturated field could exceed `maximum_intensity`; `TODO-MANA-006` closed that by clamping the running total the same way a cell fed from inside its own chunk already was. Gate calibration is settled: the response and gate constants were calibrated against a field that no carrier populated, so the gate latched open instead of discriminating; `TODO-MANA-007` closed that by recalibrating against the population the gate actually reads. The lattice question is settled: `TODO-MANA-004` closed with `chunk_extent` staying 3, on the finding that the terrain has almost no coherent structure for a finer field to resolve and that refining it makes the gate less responsive rather than more.
