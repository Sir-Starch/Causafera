...
| Geography | Hierarchy, terrain cells, deterministic terrain carrier participating in the tick loop as a standing structure, per-cell elevation and roughness projected to the observer, terrain continuous across chunk boundaries with the carrier's structure magnitude reading real cross-chunk neighbours, surface material generated as spatially coherent regions rather than per-cell noise, and ownership of the conserved hydrology carriers the hydrology domain evolves | M2 | Durable geographic processes, material provenance beyond spatial coherence, climate/ecology coupling, and multiscale synthesis |
| Geology | Documentation only | M0 | Formation, strata, deposits, deformation, material provenance, and long-timescale resolution |
| Hydrology | Conserved fixed-point surface/soil/groundwater/conveyance storage owned by geography; explicit persisted forcing records; a fixed nine-substage tick with head-driven lateral flow across same-chart chunk seams; an exactly-zero conservation residual per tick with full transfer receipts; conservative block-level resolution driven by the engine's own resolution field; causal bootstrap, persistence at digest schema 8, byte-identical round trips, and import revalidation; bounded additive observer projections and typed Explanation claims with explicit insufficiency (TODO-HYDRO-001) | M2 | Climate coupling beyond persisted forcing records, geology/permeability provenance, snow and phase change, sediment and water quality, full hydraulics, cross-chart routing, ecology/settlement coupling, and the trace-growth ceiling recorded in `docs/performance/benchmarks.md` |
| Climate | Documentation only. Hydrology's persisted forcing records are a future-compatible output boundary: they carry an opaque producer identity and a schema, and nothing behind them is simulated | M0 | Atmosphere/energy state, transport, seasonality, terrain/water/ecology coupling, and resolution |
...
| Observer | Versioned queries and bounded streams for summary, chunks, Explanation, and per-chunk field rasters with per-cell provenance; the runtime summary additionally carries a bounded six-record bootstrap receipt projection with trace anchors and no process names | M3 infrastructure | Causal slices, domain series, objective/subjective views, entity/history inspection, schema metadata, and measured overhead. The bootstrap-summary encoding cost is measured at a few hundred nanoseconds per poll (roughly 3%) at the bounded envelope, with the control and its counterpart interleaved over twenty samples; the current figures and their run-to-run spread are in the Bounded measurement section of `plans/production-bootstrap-receipt-closure.md` rather than restated here, because restating them is what made this line stale twice. That is one load level, not the four the criteria ask for |
