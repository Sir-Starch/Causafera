...
| Simulation runtime | Deterministic eight-phase harness, provenance, replay, persistence, bounded observer; modular architecture with extracted domain modules (INV-042) | M3 infrastructure; domain depth uneven | Remove fixtures/timers, integrate all claimed domains, durable coupling, maturity validation, provenance growth strategy, and representative performance. `plans/performance-baseline-and-digest-cost.md` (Accepted) root-caused the provenance-growth-strategy and representative-performance gaps: an unbounded per-tick full-rescan `history_digest`/`physical_state_digest` in `RuntimeState::snapshot`, and a `RuntimeConfig` validation gap that admitted `actor_count`/`sensor_count` combinations `MAX_SCENE_CUES` cannot execute. The validation gap is closed — `validate()` now rejects past a worst-case per-actor cue bound derived from the perception code — and a checked-in harness reproduces the digest measurements; the digest growth itself is still open |
