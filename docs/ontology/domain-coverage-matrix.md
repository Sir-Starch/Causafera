...
| Matter | Generic physical/material contracts; chart-qualified material surfaces with durable condition state; a bounded, conserved retained-heat exchange with the thermal carrier (TODO-THERMAL-002) | M1–M2 | Conserved transformations beyond retained heat, contact/containment coupling, production mutation systems, broader material provenance, expansion/damage/phase-change response to retained heat (TODO-THERMAL-007), and heterogeneous per-material thermal properties (TODO-THERMAL-008) |
| Energy | Fixed-point thermal-energy carrier with conserved same-chart intra-chunk and cross-chunk transfer, finite historical-bootstrap reservoirs, exact conservation accounting, persisted current-batch active-region boundary records, and a bounded conserved exchange with co-located material surfaces (TODO-THERMAL-002) | M2–M3 | Cross-chart transport, climate/biology integration, economy, and the material-response follow-ups above |
