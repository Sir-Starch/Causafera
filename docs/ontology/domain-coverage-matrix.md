...
| Geography | Hierarchy, terrain cells, deterministic terrain carrier participating in the tick loop as a standing structure | M1–M2 | Durable geographic processes, materials, water, climate/ecology coupling, multiscale synthesis, and observer detail |
...
| UI | Real dark Tauri/React observer with bounded aggregate views and persistent five-language localization (`en`, `ru`, `zh-Hans`, `de`, `es`) | M2 presentation | Batch stable inspection workflows after simulation/Explanation contracts mature; do not chase every internal field |
