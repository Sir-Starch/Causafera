...
**Evidence:** `elevation_contrast`, `material_difference` and `neighbor_indices` in `crates/causafera-runtime/src/carrier.rs` index only `terrain.elevations()`/`terrain.surface_materials()` of the one `TerrainChunk` passed in, and `neighbor_indices` drops a direction entirely at `x == 0`, `x == CHUNK_SIZE - 1`, `y == 0` or `y == CHUNK_SIZE - 1` rather than looking into the neighbouring chunk. `TODO-GEO-005` made elevation itself continuous across a chunk boundary; this computation still cannot see that continuity, so an edge cell's `structure` — and therefore the standing carrier's mana contribution near a chunk edge — is drawn from at most 2-3 real neighbours where an interior cell has 4. Found while implementing `TODO-GEO-005`, deferred there as a distinct defect (INV-043: the engine must not rely on chunk boundaries as absolute physical barriers)
