...
- Released CLA version 1.1: durable maintainer identity anchored to a GitHub user ID on a fixed date, a narrowed Contribution definition excluding issues and discussion, an acceptance-scoped copyright grant, a patent grant directed at the maintainer and recipients, per-release irrevocable public licensing without a hosting obligation, service-realistic acceptance records, a disclaimer that no longer negates the contributor's own provenance representations, and a no-governance-rights clause. The document carries no operational status and uses absolute links, so the signable text stays stable and Gist-safe;
- Replaced the blanket prohibition on external contributions with a CLA-gated policy: external contributions may be merged after CLA acceptance, required checks, and maintainer approval;
- Brought the CLA acceptance workflow into service and closed `TODO-LEGAL-001`. CLA 1.1 is published as a public Gist (`eb32d78ea648f989831f7aa0a3bac81c`, revision `7c6daa72020318c47d14bca27655097cce236d6b`) byte-identical to `CLA.md`; CLA Assistant is linked to `Sir-Starch/Causafera` against it with Shared Gist disabled and no minimum-change exemption; `license/cla` is a required status check on `main` alongside `rust` and `ui`, with no ruleset bypass actors; and the maintainer plus non-signing automation accounts are exempt through the service's own allowlist rather than a branch-protection bypass, leaving enforcement intact for everyone else;
