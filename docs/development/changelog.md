...
- Replaced the blanket prohibition on external contributions with a staged policy: external pull requests may be prepared and discussed, but cannot be merged until the CLA acceptance workflow is verified end to end;
- Published CLA 1.1 as a public Gist (`eb32d78ea648f989831f7aa0a3bac81c`, revision `7c6daa72020318c47d14bca27655097cce236d6b`) verified byte-identical to `CLA.md`, and linked CLA Assistant to `Sir-Starch/Causafera` against it with Shared Gist disabled and no minimum-change exemption. The workflow is configured but not verified — no test pull request, acceptance record, required check, bot verification, or export yet — so merging external contributions remains blocked and `TODO-LEGAL-001` moves to In Progress;
